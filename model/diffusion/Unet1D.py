...
class Unet1D(nn.Module):
    def __init__(
        ...
        mid_dim = dims[num_layers]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)

        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[:num_layers])):
            is_last = ind == (num_layers - 1)
...
class Unet1D_crossatt(nn.Module):
    def __init__(
        self,
        dim,
        num_layers,
        ...
        if context_dim is not None:
            assert use_crossatt, 'Fool!! You forgot to use the spatial transformer for your cross-attention conditioning..'
        self.context_dim = context_dim
        self.use_crossatt = use_crossatt
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out[:num_layers]):
            is_last = ind >= (num_layers - 1)
            layers = nn.ModuleList([])  # Create a new ModuleList for each group

            # Add blocks to the layers list
            layers.append(block_klass(dim_in, dim_in, time_emb_dim=time_dim))
            layers.append(block_klass(dim_in, dim_in, time_emb_dim=time_dim))
            ...
            layers.append(Downsample(dim_in, dim_out) if not is_last else nn.Conv1d(dim_in, dim_out, 3, padding=1))

            # Add the group of layers to the downs list
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[:num_layers])):
            ...
            # Add blocks to the layers list
            layers.append(block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim))
            layers.append(block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim))
            ...
            layers.append(Upsample(dim_out, dim_in) if not is_last else nn.Conv1d(dim_out, dim_in, 3, padding=1))

            # Add the group of layers to the ups list
...
if __name__=="__main__":
    ...
    t = torch.randint(0, 1000, (128,)).long()
